...
export function MediaStudioStep({
  ...
  const addFiles = (incoming: File[]) => {
    ...
    const files = accepted.slice(0, Math.max(0, 30 - value.length))
  ...
  return (
    <section className={styles.stepSection} aria-labelledby="media-step-title">
      <header className={styles.stepHeader}>
        <div>
          <p className={styles.kicker}>Adım 3 / 5</p>
          <h1 id="media-step-title" tabIndex={-1}>Fotoğraf stüdyosu</h1>
          <p>
            Alıcıların mülkü hızlıca anlaması için aydınlık, güncel ve doğru
            sıralanmış fotoğraflar ekleyin.
          </p>
        </div>
        <span className={styles.requiredNote}>En az 3 · En fazla 30</span>
      </header>

      <div className={styles.mediaSummary}>
        <div>
          <strong>{value.length}/30</strong>
          <span>fotoğraf</span>
        </div>
        <p>
          İlk görsel kapak olur. Cepheyi, ana yaşam alanını ve çevreyi ilk sıralara
          taşımanız önerilir.
        </p>
      </div>
      ...
      >
        ...
        <span>
          ...
          <small id={helpId}>JPG, PNG veya WebP · Görselleri sonradan sıralayabilirsiniz</small>
        </span>
      </label>
      ...
      {value.length > 0 ? (
        <ol className={styles.mediaGrid} aria-label="Fotoğraf sırası">
              ...
              <div className={styles.mediaCardBody}>
                ...
                <div className={styles.mediaActions}>
                  {item.status === 'ready' && !item.isCover ? (
                    ...
                    <span>Seçili kapak</span>
                  ) : (
                    <span>Kapak için uygun değil</span>
                  )}
                  <div>
                    {item.status !== 'ready' ? (
                      <button
        ...
        <div className={styles.emptyMedia}>
          <p className={styles.contextEyebrow}>Önerilen fotoğraf sırası</p>
          <ol>
