...
export function GirisPage() {
  ...
  const gonder = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // `to: '/giris/kod'` henüz kayıtlı bir rota değil (bkz. Task 6) — Task 2'deki
    // `useKorumaliRota` ile aynı `href` deseni kullanılır; tip denetimi rota
    // kayıt defterini zorunlu kılmaz, `donus` yine `guvenliDonusYolu`'ndan geçer.
    navigate({
      href: `/giris/kod?donus=${encodeURIComponent(guvenliDonusYolu(donus))}`,
    })
  }
