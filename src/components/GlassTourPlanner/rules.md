...
| onay butonu | ✅ (her zaman) | "Planı Onayla" | `onConfirm` yoksa/`loading`sa `disabled`, asla gizlenmez |

Children kabul edilmez — tamamen prop güdümlü (`GlassMatchScore`/
`GlassTourScheduler` ile aynı karar).
...
| onConfirm | prop | `() => void` | — | — | "Planı Onayla" tıklanınca çağrılır; verilmezse buton `disabled` (gizlenmez) |
...
| ...rest | — | `HTMLAttributes<HTMLElement>` | — | — | `className`/`style` birleştirilir, köke (`<section>`) uygulanır |
...
|---|---|
| `stops` boş | Rota render edilmez, sabit metin: "Bu tur planında henüz durak eklenmemiş." |
...
| onay butonu aktifliği | `!onConfirm \|\| loading` | tıklanabilirlik | native `disabled` |
...
- "Planı Onayla": tıklamada YALNIZ `onConfirm?.()` çağrılır — component
  ...
  `onConfirm` verilmemişse buton `disabled` — component sahte bir "başarı"
  göstermez, ama AI çıktısının onay affordance'ı görsel olarak asla
  kaybolmaz (AI-first ilkesi: kullanıcı her zaman "bunu onaylayabilirim"
  seçeneğini görür, gerçekte tıklanabilir olup olmaması ayrı bir katman).
- `loading`: skeleton içeriği tamamen `aria-hidden`; `role="status"` düğümü
  ekranokuyucuya tek seferlik "Tur planı hazırlanıyor" duyurur. Skeleton
  animasyonu yalnız `opacity` (shimmer/gradient yok, kendi flat
  placeholder'ı).
